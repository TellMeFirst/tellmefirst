...
public class DBpediaManager {
	...
	public String[] getCoordinatesForAPlace(String uri) {
		...
		try {
			...
			String cleanUri = uri.replace("%28", "(").replace("%29", ")")
					.replace("%27", "'");
			ResultSet resultSet = jenaSparqlManager
					.executeRemoteQuery(
	...
	// this method returns an empty string when a sameAs between DBpedia and NYT
	// is not available
	public String getNytUri(String dbpediaUri) {
		...
		try {
			JenaSparqlManager jenaSparqlManager = new JenaSparqlManager();
			String cleanUri = dbpediaUri.replace("%28", "(")
	...
	public String getArtistFromEnglishDBpedia(String uri) {
		...
		try {
			...
			while (resultSet.hasNext()) {
				QuerySolution row = resultSet.next();
				result = row.get("artistLabel").toString().replace("\"", "")
						.replace("@en", "");
			}
		} catch (Exception e) {
		...
		try {
			...
			ResultSet resultSet = jenaSparqlManager
					.executeRemoteQuery(
							"SELECT ?artistLabel WHERE {<"
									+ cleanUri
									+ "> <http://it.dbpedia.org/property/artista> ?artistLabel ."
	...
	public String getAbstract(String uri, String lang) {
		...
		try {
			...
			if (lang.equals("italian")) {
				language = "IT";
				endpoint = (uri.startsWith("http://it")) ? DBPEDIA_ITALIAN_ENDPOINT
						: DBPEDIA_ENGLISH_ENDPOINT;
			} else {
				language = "EN";
			...
			while (resultSet.hasNext()) {
				QuerySolution row = resultSet.next();
				result = row.get("abstract").toString().replace("@it", "")
						.replace("@en", "");
			}
		} catch (Exception e) {
	...
	public boolean isDBpediaEnglishUp() {
		...
		boolean isDBpediaEnglishUp = true;

		Element elt = CacheManager.getInstance().getCache("tmf")
				.get("isDBpediaEnglishUp");
		if (elt != null) {
			isDBpediaEnglishUp = (Boolean) elt.getObjectValue();
			LOG.debug("[isDBpediaEnglishUp] value from cache = "
					+ isDBpediaEnglishUp);
			...
			try {
				JenaSparqlManager jenaSparqlManager = new JenaSparqlManager();
				ResultSet resultSet = jenaSparqlManager.executeRemoteQuery(
						"SELECT ?s WHERE { ?s ?p ?o . } LIMIT 1",
						DBPEDIA_ENGLISH_ENDPOINT);
				if (!resultSet.hasNext()) {
					LOG.debug("[isDBpediaEnglishUp] - END");
					isDBpediaEnglishUp = false;
				}
			} catch (Exception e) {
				LOG.debug("[isDBpediaEnglishUp] - END");
				isDBpediaEnglishUp = false;
			...
			LOG.debug("########### [isDBpediaEnglishUp] took " + duration
					+ " seconds. ###########");

			CacheManager.getInstance().getCache("tmf")
					.put(new Element("isDBpediaEnglishUp", isDBpediaEnglishUp));

			LOG.debug("[isDBpediaEnglishUp] set value in cache = "
					+ isDBpediaEnglishUp);
		...
		return isDBpediaEnglishUp;
	}

	public boolean isDBpediaItalianUp() {

		LOG.debug("[isDBpediaItalianUp] - BEGIN");
		boolean isDBpediaItalianUp = true;

		Element elt = CacheManager.getInstance().getCache("tmf")
				.get("isDBpediaItalianUp");
		if (elt != null) {
			isDBpediaItalianUp = (Boolean) elt.getObjectValue();
			LOG.debug("[isDBpediaItalianUp] value from cache = "
					+ isDBpediaItalianUp);
			...
			long endTime = System.currentTimeMillis();
			long duration = (endTime - startTime) / 1000;
			LOG.debug("########### [isDBpediaItalianUp] took " + duration
					+ " seconds. ###########");

			CacheManager.getInstance().getCache("tmf")
					.put(new Element("isDBpediaItalianUp", isDBpediaItalianUp));
			LOG.debug("[isDBpediaItalianUp] set value in cache = "
					+ isDBpediaItalianUp);

		}

		LOG.debug("[isDBpediaItalianUp] - END");
		return isDBpediaItalianUp;
	}
	...
	public static void main(String[] args) {
		...
		// float[] coordinate =
		// bpm.getCoordinatesForAPlace("http://dbpedia.org/resource/Italy");
		...
		// bpm.getTypes("http://dbpedia.org/resource/Italy");
		// for(String s : types){
		// System.out.println(s);
		// }
		// String artist =
		// bpm.getArtistFromItalianDBpedia("http://it.dbpedia.org/resource/Una_mattina");
		String abstracto = bpm.getAbstract(
				"http://dbpedia.org/resource/Kevin_Costner", "italian");
