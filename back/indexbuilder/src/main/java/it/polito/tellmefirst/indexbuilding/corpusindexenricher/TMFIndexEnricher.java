 ...
 */
public class TMFIndexEnricher extends BaseIndexer<Object> {
        ...
        for (int i=0; i<indexSize; i++) {
            ...
            if (titles != null) {
                for (String title : titles) {
                    int numberOfAdds = 1;
                    for (int j=0; j<numberOfAdds; j++) {
                        doc = addField(doc, title, "TITLE");
                    }
                }
            }
            Term uriTerm = new Term(LuceneManager.DBpediaResourceField.URI.toString(), uri);
            mWriter.updateDocument(uriTerm, doc);
            commit(i);
        }
        done(indexSize);
    }

    public void enrichWithImagesEnglish(Map<String,LinkedHashSet<String>> imagesMap) throws SearchException, IOException, IndexException {
        long indexSize = searcher.getNumberOfEntries();
        if (indexSize == 0) {
            throw new IllegalArgumentException("index in "+mLucene.directory()+" contains no entries; this method can only add types to an existing index");
        }
        LOG.info("Adding images to index "+mLucene.directory()+"...");

        if (imagesMap == null) {
            LOG.error("Images map was empty. Done.");
            return;
        }
        for (int i=0; i<indexSize; i++) {
            Document doc = searcher.getFullDocument(i);
            String uri = doc.getField(LuceneManager.DBpediaResourceField.URI.toString()).stringValue();
            LinkedHashSet<String> images = imagesMap.get(uri);
            if (images != null) {
                for (String title : images) {
                    int numberOfAdds = 1;
                    for (int j=0; j<numberOfAdds; j++) {
                        doc = addField(doc, title, "IMAGE");
                    }
                }
            }
            Term uriTerm = new Term(LuceneManager.DBpediaResourceField.URI.toString(), uri);
    ...
    public void enrichWithSameAs(Map<String,LinkedHashSet<String>> sameAsMap) throws SearchException, IOException, IndexException {
        long indexSize = searcher.getNumberOfEntries();
        if (indexSize == 0) {
        ...
        for (int i=0; i<indexSize; i++) {
            Document doc = searcher.getFullDocument(i);
            String uri = doc.getField(LuceneManager.DBpediaResourceField.URI.toString()).stringValue();
            LinkedHashSet<String> sameAs = sameAsMap.get(uri);
            if (sameAs != null) {
                for (String same : sameAs) {
                    int numberOfAdds = 1;
                    for (int j=0; j<numberOfAdds; j++) {
                        doc = addField(doc, same, "SAMEAS");
                    }
                }
            }
            Term uriTerm = new Term(LuceneManager.DBpediaResourceField.URI.toString(), uri);
    ...
    public Document addField(Document doc, String fieldValue, String fieldName) {
        Field field = getField(fieldValue, fieldName);
        doc.add(field);
        return doc;
    }
